...
<cfquery name="getAmenities" dataSource="#settings.booking.dsn#">
	select id, concat(id,',',name) as displayname,name from bf_amenities order by name
</cfquery>
...
<cfif isdefined('url.amenity') and isdefined('url.btnSubmit')>
	...
	<cfquery name="getamenity" dbtype="query">
		select id,name from getAmenities where id = <cfqueryparam CFSQLType="CF_SQL_VARCHAR" value="#url.amenity#">
	</cfquery>
	...
	<cfif amenityCheck.recordcount eq 0>
		   ...
		   <cfset newString = replace(newString,'$','','all')>		
			...
			<cfquery dataSource="#settings.dsn#">
				...
				values(<cfqueryparam CFSQLType="CF_SQL_VARCHAR" value="#getamenity.name#">,<cfqueryparam CFSQLType="CF_SQL_VARCHAR" value="#getamenity.id#">)
			</cfquery>
					...
					<div class="controls">
						<select name="amenity" style="width:300px">
							...
							<cfloop query="getAmenities">
								<cfif isValid('string',displayname)>
