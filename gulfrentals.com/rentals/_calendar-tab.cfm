...
<cfif trim(property.turnday) eq "Saturday">
	<cfset turndaynum = 7>
<cfelseif trim(property.turnday) eq "Sunday">
	<cfset turndaynum = 1>
<cfelse>
	<cfset turndaynum = 7>
</cfif>

<cfquery name = "getSpecialforproperty" datasource = "#settings.dsn#">
	select *
	from cms_specials join cms_specials_properties ON(cms_specials.id = cms_specials_properties.specialID)
	  where 1=1
	  and now() between cms_specials.allowedBookingStartDate and cms_specials.allowedBookingEndDate
	...
	and cms_specials_properties.unitcode = <cfqueryparam value = "#property.propertyid#" CFSQLType = "CF_SQL_VARCHAR">
</cfquery>
  				...
  				<table class="table table-bordered calendar-table">
    				<tr><th class="month" colspan="7"><cfoutput><strong>#DateFormat(ThisMonthYear,"MMMM YYYY")#</strong></cfoutput></th></tr>
  					...
  					<tr>
						...
						<cfset formattedDateforrate = DateFormat(DateAdd('d',-1,formattedDate),'yyyy-mm-dd')>
  						...
  						<td class="text-white<cfoutput><cfif formattedDate lt #dateformat(now(),'yyyy-mm-dd')#> booked<cfelse> #myclass#</cfif></cfoutput>" width="14.28%">
  							...
  							<!--- Is this a Saturday? --->
							<cfif <!---DayOfWeek("#theYear#-#theMonth#-#x#") is 7 and---> myclass neq 'booked' and DayOfWeek("#theYear#-#theMonth#-#x#") eq turndaynum>
							<!--- Changed to Friday since rates enddates are friday --->
							<!---<cfif DayOfWeek("#theYear#-#theMonth#-#x#") is 6 and myclass neq 'booked'>--->
  								<cfset showPrice = true>
  								...
  								<!---<cfif showPrice>
									  ...
									  <cfif isDefined('getSpecialforproperty') and (getSpecialforproperty.discountPercentage neq "" or getSpecialforproperty.discountPercentage gt 0) and getSpecialforproperty.hidefromcalendar eq "No">
										<cfset getRate = getRate - getRate*getSpecialforproperty.discountPercentage>
										<!---<span>
											Price was: <sup>$</sup>#baseRate# - Price reduced to: <sup>$</sup>#specialPrice#  <small>+Taxes and Fees</small>
										  </span>--->
										<cfelseif isDefined('getSpecialforproperty') and (getSpecialforproperty.discountAmount neq "" or getSpecialforproperty.discountAmount gt 0) and getSpecialforproperty.hidefromcalendar eq "No">
											<cfset getRate = getRate - getSpecialforproperty.discountAmount>
											<!---<span>
												Price was: <sup>$</sup>#baseRate# - Price reduced to: <sup>$</sup>#specialPrice#  <small>+Taxes and Fees</small>
											  </span>--->
									  </cfif>
                      				...
                      				<cfoutput><a href="#settings.booking.bookingURL#/#settings.booking.dir#/book-now.cfm?propertyid=#property.propertyid#&strcheckin=#temp#&strcheckout=#myCheckout#" class="property-rate text-white">#NumberFormat(getRate,'$__')#</a></cfoutput>
