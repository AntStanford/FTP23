	...
	<cfloop from="1" to="#arraylen(feesArray)#" index="i">
      ...
      <!--- Add this in case the user enters a valid promo code --->
	   ...
	   <cfif description contains 'insurance'>
	   	<cfset travelinsuranceAmt = theamount>
	   </cfif>

		<tr>
		   <td>#description# <cfif theamount lt 0>(Promo Discount)</cfif></td>
		   <td><cfif theamount lt 0>$ #theamount#<cfelse>#DollarFormat(theamount)#</cfif></td>
		</tr>
		<cfif theamount gt 0>
			<cfset totalamount = totalamount + theamount>
		<cfelseif theamount lt 0>
			<cfset totalamount = totalamount + theamount>
		</cfif>
	</cfloop>
</cfif>

<cfset variables.TotalFullAmount = totalamount>
...
<script type="text/javascript">
   ...
   $('input##TotalFull').val("#variables.TotalFullAmount#");
</script>
