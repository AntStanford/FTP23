...
<cfparam name="grandtotalpaid" default="0" />
...
select strEmail as TenantEmail,dtChangeDate as UpdatedDate,strPropID as propertyid
  ...
  <div class="widget-content nopadding">
    ...
    <cfoutput query="getinfo">
      ...
      <cfif GetBooking.recordcount gt 0>
        <cfquery name="getTotal" datasource="#settings.dsn#">
        select dblAmountPaid
        from pp_guestreservationinfo
        where stremail = <cfqueryparam cfsqltype="cf_sql_varchar" value="#email#" />
        and strpropid = <cfqueryparam cfsqltype="cf_sql_varchar" value="#replace(unitcode,'+',' ','all')#" />
      ...
      <td width="45">#currentrow#. <a name="#currentrow#"></a></td>		
   		...
   		<td>
        <cfif GetBooking.recordcount gt 0>
          Yes - <cfif val( totalpaid ) gt 0>
                  <cfset grandtotalpaid += val( totalpaid ) />
                  
                  #dollarformat( totalpaid )#
                </cfif>
            <br>
          <cfif isDefined("GetBooking.propertyid")>
      ...
      <tr>
        ...
        <td colspan="2"><cfoutput>#dollarformat( grandtotalpaid )#</cfoutput></td>
      </tr>
