...
<cfquery name="getinfo" dataSource="#settings.dsn#">
  select * from cms_contacts order by firstname limit 11
</cfquery>
...
<cfoutput>#firstname#,#lastname#,#address1#,#address2#,#city#,#state#,#zip#,<cfif len(GetInfo.email)>#decrypt(GetInfo.email, application.contactInfoEncryptKey, 'AES')#</cfif>,<cfif len(GetInfo.phone)>#decrypt(GetInfo.phone, application.contactInfoEncryptKey, 'AES')#</cfif>,#DateFormat(createdat,"mm/dd/yyyy")# #TimeFormat(createdat,"hh:mm:ss")#,#camefrom##crlf#</cfoutput>
</cfloop>
</cfsavecontent>
