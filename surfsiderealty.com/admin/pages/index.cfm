...
<style type="text/css">
  ...
  .subsort,.subsubsort {cursor: pointer;}
</style>
...
<!--- <cfoutput>#settings.dsn#</cfoutput> --->
    ...
    <div class="table-wrap">
      <table class="table table-heading">
        <tr>
          ...
          <th width="170">Sub-Sub Level Pages</th>
      ...
      <ul id="sort-list">
        <cfoutput query="getinfo"><cfset usedList = listappend(usedList,id)>
          <li id="listItem_#id#">
            <table class="table table-bordered table-striped">
              <tr>
                ...
                <td width="50"><a href="form.cfm?id=#id#" class="btn btn-mini btn-block btn-primary"><i class="icon-pencil icon-white"></i> Edit</a></td>
              ...
              <cfquery name="getSubPages" dataSource="#settings.dsn#">
                select id,name,slug,showinNavigation,body from cms_pages where parentID = <cfqueryparam CFSQLType="CF_SQL_INTEGER" value="#getinfo.id#">
                and subnavParentID = 0
                order by subsort
              </cfquery>
              <cfif getSubPages.recordcount gt 0>
                <cfloop query="getSubPages"><cfset usedList = listappend(usedList,getSubPages.id)>
                  <tr>
                 ...
                 <!--- Does this sub page have its own sub page? --->
                 <cfquery name="subsubCheck" dataSource="#settings.dsn#">
                  select id,name,slug,showinNavigation,subsubsort from cms_pages where subnavParentID = <cfqueryparam  CFSQLType="CF_SQL_INTEGER" value="#getSubPages.id#"> ORDER BY subsubsort
                 </cfquery>

                 <cfif subsubCheck.recordcount gt 0>

                       <cfloop query="subsubCheck"><cfset usedList = listappend(usedList,subsubCheck.id)>
                          <tr>
                             <td width="30"></td>
                             <td width="200"></td>
                             <td style="text-align:center">
                             	<!--- Add arrows for sort - down on first, up & down on all but the last, then up only on the last --->
								<cfif subsubCheck.currentrow is 1>
                                    <i class="subsubsort icon-arrow-down" data-subnavparentid="#getSubPages.id#" data-thisid="#subsubCheck.id#" data-direction="down"></i>
                                <cfelseif subsubCheck.currentrow is not subsubCheck.recordcount>
                                    <i class="subsubsort icon-arrow-up" data-subnavparentid="#getSubPages.id#" data-thisid="#subsubCheck.id#" data-direction="up"></i>&nbsp;&nbsp;&nbsp;
                                    <i class="subsubsort icon-arrow-down" data-subnavparentid="#getSubPages.id#" data-thisid="#subsubCheck.id#" data-direction="down"></i>
                                <cfelse>
                                    <i class="subsubsort icon-arrow-up" data-subnavparentid="#getSubPages.id#" data-thisid="#subsubCheck.id#" data-direction="up"></i>
                                </cfif>
                             </td>
                             <td width="90">#subsubCheck.name#</td>
                             <td width="60" style="text-align:center;">#yesNoFormat(showInNavigation)#</td>
                             <td width="60" style="text-align:center;"><cfif len(getSubPages.body)>Y<cfelse>N</cfif></td>
                             <td width="50"><a href="form.cfm?id=#subsubCheck.id#" class="btn btn-mini btn-block btn-primary"><i class="icon-pencil icon-white"></i> Edit</a></td>
                             <td width="65"><a href="submit.cfm?id=#subsubCheck.id#&delete" class="btn btn-mini btn-block btn-danger mytooltip" data-confirm="Are you sure you want to delete this page?"><i class="icon-remove icon-white"></i> Delete</a></td>
                             <td width="60"><a href="/#subsubCheck.slug#" class="btn btn-mini btn-block" target="_blank"><i class="icon-eye-open"></i> View</a></td>
                          </tr>
                       </cfloop>
        ...
        <cfquery name="getorphans" dataSource="#settings.dsn#">
          select * from cms_pages where showInAdmin = 'Yes' and id not in (<cfqueryparam value="#usedList#" cfsqltype="cf_sql_varchar" list="true">) order by sort
        </cfquery>
          ...
          <li id="listItem_#id#">
            <table class="table table-bordered table-striped">
              <tr>
                ...
                <td width="200"></td>
                <td width="90" style="text-align:center;">#showInNavigation#</td>
                <td width="60" style="text-align:center;"><cfif len(getorphans.body)>Y<cfelse>N</cfif></td>
                <td width="50"><a href="form.cfm?id=#id#" class="btn btn-mini btn-block btn-primary"><i class="icon-pencil icon-white"></i> Edit</a></td>
                <td width="65"><a href="submit.cfm?id=#id#&delete" class="btn btn-mini btn-block btn-danger mytooltip" data-confirm="Are you sure you want to delete this page?"><i class="icon-remove icon-white"></i> Delete</a></td>
                <td width="60">
                  <cfif triggerOnly eq 'Yes'>
...
//console.log('parentId: ' + parentId + ", thisId: " + thisId + ", dir: " + dir);
  ...
  $(".subsubsort").click(function(){
    var subnavparentId = $(this).attr('data-subnavparentid');
    var thisId = $(this).attr('data-thisid');
    var dir = $(this).attr('data-direction');
//console.log('parentId: ' + parentId + ", thisId: " + thisId + ", dir: " + dir);
    // this function does the re-sorting
    $.ajax({
      type: "POST",
      url: 'sub-sub-sorter.cfm',
      data: {subnavparentId: subnavparentId, thisId: thisId, dir: dir},
      success: function(data){
        $loading.show();
        location.reload();
      }
