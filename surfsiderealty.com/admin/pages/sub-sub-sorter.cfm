...
<!--- get current order --->
<cfquery name="getSubSubPages" dataSource="#settings.dsn#">
	select id, subsubsort from cms_pages where subnavParentID = <cfqueryparam CFSQLType="CF_SQL_INTEGER" value="#form.subnavparentId#"> order by subsubsort, id
</cfquery>
...
<cfloop query="getSubSubPages">
	<cfset variables.items[i] = getSubSubPages.id />
	<cfset i += 1 />
</cfloop>
...
<!---<cfdump var="#getSubSubPages#"><cfdump var="#variables.updateItems#">--->
<!--- loop through the array, updating all the subsubsort values --->
<cfloop from="1" to="#arrayLen(variables.updateItems)#" index="i">
	<cfquery datasource="#settings.dsn#">
		UPDATE 	cms_pages 
		SET 	subsubsort = <cfqueryparam cfsqltype="cf_sql_integer" value="#i#"> 
