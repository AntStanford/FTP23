...
<cfquery datasource="#mls.dsn#" name="getInfo">
	...
	<cfif isdefined('cookie.LoggedInAgentID')>
 	...
 	<cfif LEN(session.agentkeyword)>
 		and cl_leadtracker_response.messagebody LIKE "%#session.agentkeyword#%" 
	...
	<cfif session.joinsearch eq 1>
		...
		<cfif isdefined('session.pmin') and LEN(session.pmin)>
			and cl_saved_searches.pmin >= <cfqueryparam value="#session.pmin#" cfsqltype="cf_sql_integer">
		</cfif>

		<cfif isdefined('session.pmax') and LEN(session.pmax)>
			and cl_saved_searches.pmax <= <cfqueryparam value="#session.pmax#" cfsqltype="cf_sql_integer">
		</cfif>

		<cfif isdefined('session.wsid') and LEN(session.wsid) and session.wsid neq 'any'>
			and cl_saved_searches.wsid = <cfqueryparam value="#session.wsid#" cfsqltype="cf_sql_varchar">
		</cfif> 
...
<cfquery datasource="#dsnmls#" NAME="getareas">
	...
	where mlsid = <cfqueryparam value="#mls.mlsid#" cfsqltype="cf_sql_integer">
	order by city asc
</cfquery>
