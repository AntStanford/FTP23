...
<cfif isDefined("form.enclosure") and len(trim(form.enclosure))>
	...
	<cfif not directoryExists(destination)>
	 ...
	 <!-------Copy components folder to same directory or change the path "Components.filecheck" if you move it---------------------> 
	 <!-------Replace cffile tag with 2 lines below----><!---pass in photo destination and image field name .check(uppic,"photo") in my case---->
	 
	 <cfif not isdefined('Obj')><cfset Obj = CreateObject("Component","Components.fileCheck").Init()></cfif>
	 <cfset results=Obj.Check(destination,"enclosure")><cfif len(results)><cfoutput>#results#</cfoutput><cfabort></cfif>
...
<cfmodule template="../tags/adminlayout.cfm" title="Entry Editor">
	...
	<cfif not structKeyExists(form, "preview")>
		...
		<div id="additional">	
			...
			<div class="ctrlHolder">
				Post URL:
				&nbsp; <a href="#application.rooturl#/index.cfm/#DateFormat(form.posted, "yyyy/m/d")#/#form.alias#">#application.rooturl#/index.cfm/#DateFormat(form.posted, "yyyy/m/d")#/#form.alias#</a>
			</div>
			<div class="ctrlHolder">
